...
<?php
	echo "\t\t<li>";
	echo $this->Html->link(__d('cake', 'Edit {0}', $singularHumanName), array('action' => 'edit', ${$singularVar}[$modelClass][$primaryKey]));
	echo " </li>\n";

	echo "\t\t<li>";
	echo $this->Form->postLink(__d('cake', 'Delete {0}', $singularHumanName), array('action' => 'delete', ${$singularVar}[$modelClass][$primaryKey]), array(), __d('cake', 'Are you sure you want to delete # {0}?', ${$singularVar}[$modelClass][$primaryKey]));
	echo " </li>\n";

	echo "\t\t<li>";
	echo $this->Html->link(__d('cake', 'List {0}', $pluralHumanName), array('action' => 'index'));
	echo " </li>\n";

	echo "\t\t<li>";
	echo $this->Html->link(__d('cake', 'New {0}', $singularHumanName), array('action' => 'add'));
	echo " </li>\n";
	...
	foreach ($associations as $_type => $_data) {
		foreach ($_data as $_alias => $_details) {
			if ($_details['controller'] != $this->name && !in_array($_details['controller'], $done)) {
				echo "\t\t<li>";
				echo $this->Html->link(
					__d('cake', 'List {0}', Inflector::humanize($_details['controller'])),
					array('plugin' => $_details['plugin'], 'controller' => $_details['controller'], 'action' => 'index')
				);
				echo "</li>\n";
				echo "\t\t<li>";
				echo $this->Html->link(
					__d('cake', 'New {0}', Inflector::humanize(Inflector::underscore($_alias))),
...
<?php endif; ?>
	<div class="actions">
		...
		<li><?php
			echo $this->Html->link(
				__d('cake', 'Edit {0}', Inflector::humanize(Inflector::underscore($_alias))),
...
<?php
		...
		foreach (${$singularVar}[$_alias] as ${$otherSingularVar}):
			...
			echo $this->Form->postLink(
				...
				array(),
				__d('cake', 'Are you sure you want to delete # {0}?', ${$otherSingularVar}[$_details['primaryKey']])
			);
			echo "\n";
			echo "\t\t\t</td>\n";
		echo "\t\t</tr>\n";
